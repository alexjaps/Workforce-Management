...
function getUrl() {
	if(WorkForce_Obj.Env=='SANDBOX'){
		...
		return {
				'RT-Old': '/app/site/hosting/restlet.nl?script=1307&deploy=1',
...
function CleanResponseData(data){
	...
	var isError = String(data).substring(0,15);
	if (isError == '<!DOCTYPE html>'){
		console.error(data);
		return [];
	}
	...
	$.ajax({
		...
		complete: function(data) {
			...
			setTimeout(function() {
				...
				/*
				 * Comentado por Ariel 2017-02-08 5:08am
...
function ChangeTableTasksParams(arrayParams, Pm) {
	...
	if (WhoFilterDurationFrom== '' || WhoFilterDurationFrom == 0) {
	...
	$.ajax({
		...
		error: function(err) {
			console.error("Internal Error on Call ",err);
		}
	});
	return false;
...
function formatDate(date) {
	if(WF_DEBUG)
		console.log('formatDate (param)',date);
...
function openWrikeComment (wrikeid, Taskid, fromtree) {
	...
	if (!wrikeid) {
		...
		// }else{
		// 	alert("You don't have your wrike account syncronized with netsuite \n On the next window click in \"Request Wrike Access\"");
		// 	url = '/app/crm/calendar/task.nl?id='+ Taskid + '&whence=';
...
function UpdateStartByDependency(idInput, doSplit) {
	//First step : Get values from predecessor object
	if (WF_DEBUG)
...
//-- Functions Three>
function UpdatewithDurationbyDependency_3(Input) {
	//First step : Get values from predecessor object
	var arrayString = String(Input).split('_');
	var p1 = arrayString[1];
	var num = arrayString[2];
...
////////////////////////////////
function UpdateDurationbyStart_3(idInput, WorkWeekend) {
	var arrayString = String(idInput).split('_');
	var p1 = arrayString[1];
	var num = arrayString[2];
	var idDueDate = '#idPicker_DueDate_' + num;
...
//-- Functions Four>
function EnabledAndDisabledDateFilter() {
...
function SetTableTasksByFilters(bybutton) {
	if(WF_DEBUG)
...
$(document).ready(function() {
	...
	loggedUserID = $('#custpage_userid').val();
...
//---RESTCODE>
	window.setTimeout(function() {
		...
		$('body').on('click', '.tabso3', function(e) {
			...
			if (!($.fn.dataTable.isDataTable('#tableSOcount'))) {
				...
				$.ajax({
					...
					error: function(err) {
						console.log('Internal Error on Call',err);
					},
					complete: function() {
		...
		//-- Functions TWO Whas Here .... leirags


		//-- Functions THREE-- was here---leirags
		...
		//-- Function FOUR was here --- leirags
