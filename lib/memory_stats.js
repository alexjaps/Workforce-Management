 ...
 * 
 * 
 * Include in your page this code:
 * (function() {
...
var MemoryStats = function (){
	...
	if( performance.memory.totalJSHeapSize === 0 ){
		...
		return {notrun:true, update: function(){} };
	}
	...
	container.id	= 'stats';
	container.style.cssText = 'width:120px;height:48px;opacity:0.9;cursor:pointer;overflow:hidden;z-index:10000;will-change:transform;';
	...
	msGraph.id	= 'msGraph';
	msGraph.style.cssText = 'position:relative;width:114px;height:' + GRAPH_HEIGHT + 'px;background-color:#0f0';
	...
	var dorun =  (typeof performance.memory === 'object');
	// support of the API?
	if( performance.memory.totalJSHeapSize === 0 ){
		console.warn('totalJSHeapSize === 0... performance.memory is only available in Chrome .');
		dorun = false;
	...
	var lastUsedHeap = (dorun) ? performance.memory.usedJSHeapSize : 0;
	var delta = 0;
	var color = "#131";
	var mbValue = 0;
	var ms = 0;
	...
	return {
		...
		update: function () {
			...
			// update at 30fps
			if( Date.now() - lastTime < 1000/20 ) return;
			lastTime = Date.now();
		...
		info: function (){
			...
			return {
				'lastUsedHeap' : lastUsedHeap,
				'currUsedHeap' : performance.memory.usedJSHeapSize,
			}
		}
...
// Last Review - 2017-Feb-11 01:17PM